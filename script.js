document.addEventListener('DOMContentLoaded', () => {
    ...
    function confirmLine() {
        ...
        // 【邏輯修改】
        let newSegmentShared = false; // 用於追蹤是否 "新共享" 了線
            ...
            if (lines[id]) {
                if (!lines[id].drawn) { 
                    ...
                    if (lines[id].sharedBy === 0) {
                        ...
                        newSegmentShared = true;
                    }
                }
            }
        }

        // 5. 【邏輯修改】 如果沒有畫到任何新線 "且" 沒有新共享任何線，才顯示錯誤
        if (!newSegmentDrawn && !newSegmentShared) {
            alert("這條線 (或所有部分) 已經被畫過了 (或已被您共享)。");
            cancelLine();
            return;
        }
    ...
    function makeAIMove() {
        ...
        if (bestMove && bestMove.dot1 && bestMove.dot2) {
            ...
            // 【邏輯修改】
            let newSegmentShared = false; // AI 也需要檢查共享
            ...
            for (const id of segmentIds) {
                if (lines[id]) {
                    if (!lines[id].drawn) { 
                        lines[id].drawn = true;
                        lines[id].player = currentPlayer; // AI 是 2
                        newSegmentDrawn = true;
                    } else if (lines[id].player !== 0 && lines[id].player !== currentPlayer) {
                        // 【邏輯修改】
                        if (lines[id].sharedBy === 0) {
                            lines[id].sharedBy = currentPlayer; // AI 是 2
                            newSegmentShared = true;
                        }
                    }
                }
    ...
    function findBestAIMove() {
        ...
        if (scoringMoves.length > 0) {
            ...
            if (bestAndSafeMoves.length > 0) {
                ...
                // 1c. 如果得分的同時「一定會」幫對手搭橋，那也只能選了 (至少有得分)
        ...
        // 4. (理論上不會到這一步，因為 evaluatedMoves > 0)
        // 但以防萬一，隨便回傳一個
        ...
        for (let i = 0; i < allDots.length; i++) {
            for (let j = i + 1; j < allDots.length; j++) {
                ...
                if (allDotsOnLine.length === 4) {
                    ...
                    // 【邏輯修改】 檢查所有線段是否存在 且 (至少有一條未畫 OR 至少有一條可共享)
                    const segmentIds = [];
                    let hasUndrawnSegment = false; // 是否有虛線
                    let hasShareableSegment = false; // 是否有 "可共享" 的線
                    
                    for (let k = 0; k < allDotsOnLine.length - 1; k++) {
                        const id = getLineId(allDotsOnLine[k], allDotsOnLine[k+1]);
                        if (!lines[id]) {
                            segmentIds.length = 0; // 中斷，線不存在
                            break;
                        }
                        segmentIds.push(id);
                        ...
                        // 檢查是否為對手的線 (player 1) 且 "尚未" 被 AI (player 2) 共享
                        else if (lines[id].player === 1 && lines[id].sharedBy === 0) { 
                            hasShareableSegment = true;
                        }
                    }
                    
                    // 【邏輯修改】 必須有 3 格，且 (有新線可畫 OR 有新線可共享)
                    if (segmentIds.length === 3 && (hasUndrawnSegment || hasShareableSegment)) {
