        ...
        for (let i = 0; i < allDots.length; i++) {
            for (let j = i + 1; j < allDots.length; j++) {
                ...
                if (allDotsOnLine.length === 4) {
                    ...
                    let hasShareableSegment = false;
                    let allSegmentsValid = true; // 【新】 增加旗標
                    
                    for (let k = 0; k < allDotsOnLine.length - 1; k++) {
                        const id = getLineId(allDotsOnLine[k], allDotsOnLine[k+1]);
                        
                        // 【修復】 這是導致崩潰的關鍵
                        // 必須檢查 id 是否存在 "且" lines[id] 是否存在
                        if (!id || !lines[id]) { 
                            allSegmentsValid = false; // 標記此路徑無效
                            break; // 停止檢查這 3 格
                        }
                        
                        // --- 只有 lines[id] 存在時，才會執行到這裡 ---
                        segmentIds.push(id);
                    ...
                    // 【修復】
                    // 必須 3 格都有效，且 (有新線可畫 OR 有新線可共享)
                    if (allSegmentsValid && (hasUndrawnSegment || hasShareableSegment)) {
